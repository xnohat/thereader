...
if($_GET['safari'] == 'true'){ //isSafari
  if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
    //windows
    passthru('ffmpeg.exe -y -loglevel quiet -i '. $file .' -filter:a "atempo='. $_GET['speed'] .'" '. $fileconverted);
  } else {
    //linux
    passthru('./ffmpeg -y -loglevel quiet -i '. $file .' -filter:a "atempo='. $_GET['speed'] .'" '. $fileconverted);
  }
}else{
  $fileconverted = $file;
}
